...
import (
	...
	"generics-exercise/pkg/products"
)
...
func main() {
	...
	validateprdToStore := products.Product{
		Title:       "product title",
	...
	prd, err = validatorPrdStore.Store("prd-id-2", validateprdToStore)
	fmt.Printf("Store(%+v) = prd(%+v), error(%v)\n", validateprdToStore, prd, err)
}
